define({
    ...
    '<div id="active-org-menu" class="nav navbar-right dropdown">' +
        '<a id="active-org-menu-link" href="#" class="dropdown-toggle btn btn-default btn-sm navbar-btn" data-toggle="dropdown" title="Organización">' +
            '<i class="glyphicon glyphicon-globe"></i> {{activeOrg ? activeOrg.pseudoId : ""}}</a>' +
        '<ul v-if="userOrgList" class="dropdown-menu">' +
            '<li v-if="activeOrg"><a @click.prevent="updateActive(null)">Limpiar Organización Activa</a></li>' +
